...
public class Advancements {
	...
	public static Advancement GENERIC_BEATDEV;
	...
	public static void register() {
		...
		GENERIC_BEATDEV = new Advancement("bending/beatdev", "Better than the Developer", "Beat the developer with your bending", skull).setParent(GENERIC_RECHOOSE).setHidden(true);
		
		GENERIC_KILL_PLAYER = new Advancement("bending/kill/player", "Beginner", "Kill a player with your bending", new ItemStack(Material.WOOD_SWORD)).setParent(GENERIC_CHOOSE);
		GENERIC_KILL_PLAYER_2 = new Advancement("bending/kill/player2", "Novice", "Kill ten players with your bending", new ItemStack(Material.IRON_SWORD)).setParent(GENERIC_KILL_PLAYER).setLevel(AdvancementLevel.NORMAL).setCriteriaAmount(10);
		GENERIC_KILL_PLAYER_3 = new Advancement("bending/kill/player3", "Expert", "Kill one hundred players with your bending", new ItemStack(Material.GOLD_SWORD)).setParent(GENERIC_KILL_PLAYER_2).setLevel(AdvancementLevel.CHALLENGING).setCriteriaAmount(100);
		GENERIC_KILL_PLAYER_4 = new Advancement("bending/kill/player4", "Master", "Kill five hundred players with your bending", new ItemStack(Material.DIAMOND_SWORD)).setParent(GENERIC_KILL_PLAYER_3).setLevel(AdvancementLevel.LEGENDARY).setCriteriaAmount(500);
		...
		GENERIC_KILL_MOB_4 = new Advancement("bending/kill/mob4", "Exterminator", "Kill one thousand mobs with your bending", new ItemStack(Material.SKULL_ITEM, 1, (short) 5)).setParent(GENERIC_KILL_MOB_3).setLevel(AdvancementLevel.LEGENDARY).setCriteriaAmount(1000);
		...
		FIRE_KILL_PLAYER_5 = new Advancement("fire/kill5", "Grandmaster Firebender", "Kill five thousand players with your firebending", new ItemStack(Material.DIAMOND_SWORD)).setParent(FIRE_KILL_PLAYER_4).setLevel(AdvancementLevel.LEGENDARY).setCriteriaAmount(5000);
		FIRE_KILL_PLAYER_6 = new Advancement("fire/kill6", "Supreme Firelord", "Kill ten thousand players with your firebending!", new ItemStack(Material.DIAMOND_SWORD)).setParent(FIRE_KILL_PLAYER_5).setLevel(AdvancementLevel.LEGENDARY).setCriteriaAmount(10000).setHidden(true);
		...
		FIRE_VOIDSAVE = new Advancement("fire/voidsave", "Lifesaver", "Recover from falling in the void", new ItemStack(Material.INK_SACK, 1, (short)1)).setParent(FIRE_COMBO);
		...
		FIRE_PORTAL = new Advancement("fire/portallight", "Hell Fire", "Light a nether portal with your firebending", new ItemStack(Material.OBSIDIAN)).setParent(FIRE_CHOOSE);
		...
		FIRE_CONTROL = new Advancement("fire/control", "Under Control", "Extinguish yourself", new ItemStack(Material.COAL)).setParent(FIRE_PORTAL);
		...
		WATER_COMBO = new Advancement("water/combo", "Rapid Water", "Perform a waterbending combo", new ItemStack(Material.STAINED_CLAY, 1, (short)11)).setParent(WATER_CHOOSE);
		WATER_VOIDSAVE = new Advancement("water/voidsave", "Lifesaver", "Recover from falling in the void", new ItemStack(Material.INK_SACK, 1, (short)1)).setParent(WATER_COMBO);
		...
		WATER_SWIMMER = new Advancement("water/fastswim", "Mermaid in the making", "Swim over 1000m with fastswim", new ItemStack(boots)).setParent(WATER_CHOOSE);
		...
		WATER_ENVIROMENTALIST = new Advancement("water/environmentalist", "Nature Lover", "Use 10 plants as a source of water", new ItemStack(Material.LEAVES, 1, (short)0)).setParent(WATER_BUBBLE);
		...
		EARTH_KILL_PLAYER = new Advancement("earth/kill1", "Beginner", "Kill one player with your earthbending", new ItemStack(Material.WOOD_SWORD)).setParent(WATER_CHOOSE);
		EARTH_KILL_PLAYER_2 = new Advancement("earth/kill2", "Novice", "Kill ten players with your earthbending", new ItemStack(Material.STONE_SWORD)).setParent(WATER_KILL_PLAYER).setLevel(AdvancementLevel.NORMAL).setCriteriaAmount(10);
		EARTH_KILL_PLAYER_3 = new Advancement("earth/kill3", "Expert", "Kill fifty players with your earthbending", new ItemStack(Material.IRON_SWORD)).setParent(WATER_KILL_PLAYER_2).setLevel(AdvancementLevel.CHALLENGING).setCriteriaAmount(50);
		EARTH_KILL_PLAYER_4 = new Advancement("earth/kill4", "Master Waterbender", "Kill two hundred players with your earthbending", new ItemStack(Material.GOLD_SWORD)).setParent(WATER_KILL_PLAYER_3).setLevel(AdvancementLevel.LEGENDARY).setCriteriaAmount(200);
		EARTH_KILL_PLAYER_5 = new Advancement("earth/kill5", "Legendary Waterbender", "Kill five hundred players with your earthbending", new ItemStack(Material.DIAMOND_SWORD)).setParent(WATER_KILL_PLAYER_4).setLevel(AdvancementLevel.LEGENDARY).setCriteriaAmount(500);
		
		EARTH_COMBO = new Advancement("earth/combo", "Rigid Earth", "Perform a waterbending combo", new ItemStack(Material.STAINED_CLAY, 1, (short)5)).setParent(EARTH_CHOOSE);
		...
		EARTH_FALLKILL = new Advancement("earth/fallkill", "Sky high", "Kill a player by blasting them in the air with catapult", new ItemStack(Material.GRASS)).setParent(EARTH_CHOOSE);
		EARTH_TREEDEATH = new Advancement("earth/treedeath", "Too many leaves", "Die from catapulting up and landing on a tree", new ItemStack(Material.LOG)).setParent(EARTH_FALLKILL);
